...
namespace MESSystem.common
{
	public class productslitlistDB : mySQLClass
	{
		...
		private const int SMALL_INDEX_INDEX = 10;
		private const int CUSTOMER_INDEX_INDEX = 11;
		private const int ERROR_STATUS_INDEX = 12;
		private const int NUM_OF_JOINS = 13;
		private const int WORKER_ID_INDEX = 14;
		private const int TOTAL_DATAGRAM_NUM = WORKER_ID_INDEX+1;

		private const string c_dbName = "globaldatabase";
        private const string c_productprintlistTableName = "productslitlist";
        private const string c_productprintlistFileName = "..\\..\\data\\globalTables\\productPrintList.xlsx";

		public struct productslitlist_t{
			string machineID;
		...
		public productslitlist_t? parseinput(string strInput)
		{
			string[] input;
			productslitlist_t st_productslit;
			...
			st_productslit.batchNum = input[BATCH_NUM_INDEX];
			st_productslit.dispatchCode = input[DISPATCH_CODE_INDEX];
			st_productslit.largeIndex = input[LARGE_INDEX_INDEX];
			st_productslit.machineID = input[MACHINE_ID_INDEX];
			st_productslit.materialBarCode = input[MATERIAL_BARCODE_INDEX];
			st_productslit.materialScanTime = input[MATERIAL_SCAN_TIME_INDEX];
			st_productslit.productBarCode = input[PRODUCT_BARCODE_INDEX];
			st_productslit.productScanTime = input[PRODUCT_SCAN_TIME_INDEX];
			st_productslit.salesOrderCode = input[SALES_ORDER_CODE_INDEX];
			st_productslit.workerID = input[WORKER_ID_INDEX];
			
			return st_productslit;
		}
