...
//            gVariable.nextSerialNumber = 0;
            ...
            gVariable.maxMachineNum = gVariable.machineNameArray.Length;

            gVariable.machineNameArrayForSelection = new string[gVariable.machineNameArray.Length + 1]; //gVariable.packingMachineNameArray.Length + 1];

            for (i = 0; i < gVariable.machineNameArray.Length; i++)
            {
                gVariable.machineNameArrayForSelection[i] = gVariable.machineNameArray[i];
            }
            gVariable.machineNameArrayForSelection[i] = "所有设备";
...
//            if (gVariable.debugMode != 0)
            ...
            //for alarm list view
            gVariable.MACHINE_NAME_ALL_FOR_SELECTION = gVariable.machineNameArrayForSelection.Length - 1;
...
//                    Console.Write(DateTime.Now + "  --  ");
        ...
        private void hgToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            commonListview.commonListviewClass = new commonListview(gVariable.basicInfoDatabaseName, gVariable.employeeTableName, gVariable.employeeFileName, "员工信息", 0);
            commonListview.commonListviewClass.Show();
