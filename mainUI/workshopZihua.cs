...
//        int[] percentageArray = new int[PIC_NUM2];
        ...
        private void setTooltipData(int buttonIndex)
        {
            ...
            str = null;
            myBoardIndex = gVariable.allMachineIDForZihua[buttonIndex] - 1;
...
//                flag = 0;
                for (i = 0, j = 0; i < buttonArray.Length; i++, j++)
                {
                    ...
                    dName = gVariable.DBHeadString + gVariable.allMachineIDForZihua[j].ToString().PadLeft(3, '0');
...
//            Rectangle rect = new Rectangle();
        ...
        private void room_Selected(int i)
        {
            if (gVariable.mainFunctionIndex == gVariable.PRODUCT_CURRENT_STATUS)
            {
                ...
                {
                    if (i > gVariable.allMachineIDForZihua.Length)
                        return;

                    gVariable.currentCurveDatabaseName = gVariable.DBHeadString + gVariable.allMachineIDForZihua[i - 1].ToString().PadLeft(3, '0'); ;
                }
