...
namespace MESSystem.materialManagement
{
    public partial class StackAndFeedbin : Form
    {
        ...
        string[] machineList;
        string[] workerIDList;
        string[] feedbinIDList = { "未选择", "1号料仓", "2号料仓", "3号料仓", "4号料仓", "5号料仓", "6号料仓", "7号料仓", "8号料仓" };
        ...
        private void displayMaterialList()
        {
            ...
            //int nameIndex;
            string name;
            int[] materialLenArray = { 1, 45, 60, 120, 100, 100, 60, 130, 90, 60, 60, 130, 90, 80, 110 };
            string[] materialColumnArray = 
            {
                "", "序号", "记录类型", "物料编码", "物料名称", "目标设备", "料仓编号", "配送时间", "配送数量", "配送员工", "码垛余料", "投料时间", "投料人", "投料数量", "工单编码", 
            };
            ...
            try
            {
                ...
                if (materialListArray != null)
                {
                    ...
                    for (i = 0; i < materialListArray.GetLength(0); i++)
                    {
                        ...
                        if (materialListArray[i, 5] == "0")
                        {
                            OptionItem.BackColor = Color.LightGreen;
                            OptionItem.SubItems.Add("投料");
                        }
                        else if (materialListArray[i, 3] == "1")
                        {
                            OptionItem.BackColor = Color.SeaGreen;
                            OptionItem.SubItems.Add("配送");
                        }
                        else
                        {
                            OptionItem.BackColor = Color.LightYellow;
                            OptionItem.SubItems.Add("退仓");

                        }
                        OptionItem.SubItems.Add(materialListArray[i, 1]);
                        OptionItem.SubItems.Add(materialListArray[i, 2]);
                        OptionItem.SubItems.Add(materialListArray[i, 6]);
                        OptionItem.SubItems.Add(materialListArray[i, 7] + "号料仓");
                        OptionItem.SubItems.Add(materialListArray[i, 4]);
                        if (materialListArray[i, 5] == "0")
                        {
                        ...
                        {
                            OptionItem.SubItems.Add("配送" + materialListArray[i, 5] + "袋");
                        }
                        else
                        {
                            OptionItem.SubItems.Add("退回" + materialListArray[i, 5] + "袋");
                        }
                        if (toolClass.isDigitalNum(materialListArray[i, 8]) == 1)
                        {
                            name = toolClass.getNameByIDAndIDByName(null, materialListArray[i, 8]);
                        }
                        else
                        {
                            name = "未知";
                        ...
                        OptionItem.SubItems.Add(materialListArray[i, 9] + "袋");
                        OptionItem.SubItems.Add(materialListArray[i, 10]);
                        OptionItem.SubItems.Add(materialListArray[i, 11]);
                        if (materialListArray[i, 5] == "0")
                            OptionItem.SubItems.Add(materialListArray[i, 12] + "袋");
                        else
                            OptionItem.SubItems.Add("");
                        OptionItem.SubItems.Add(materialListArray[i, 13]);
        ...
        string [,] getMaterialInventoryList()
        {
            ...
            tableName = gVariable.materialInventoryTableName;
        ...
        void getMaterialListByCondition()
        {
            ...
            databaseName = gVariable.globalDatabaseName;
            tableName = gVariable.materialInOutRecordTableName;
