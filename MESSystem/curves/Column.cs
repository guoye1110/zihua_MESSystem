...
namespace MESSystem.curves
{
    ...
    {
        int closeReason;
        public static Column columnClass = null; //用来引用主窗口
...
//                chart.ChartAreas[0].AxisY.IsStartedFromZero = true; // false;  //whether we need to start at 0 for Y axis
                ...
                if (gVariable.dataNumForCurve[index] != 0)
                {
                    for (i = 0; i < 5; i++)
                    {
                        pointIndex = chart.Series[0].Points.AddXY(gVariable.columnLimits[index, i], gVariable.columnData[index, i] + 1);
                        dataPoint = chart.Series[0].Points[pointIndex];
                        dataPoint.LegendText = gVariable.columnLimits[index, i].ToString("f4");
...
//                    gVariable.curveTextArray[i] = null;
                ...
                this.Text = gVariable.programTitle + "生产状况监测之直方图";
...
//            mySQLClass.readSmallPartOfDataToArray(gVariable.currentCurveDatabaseName, gVariable.totalPointNumForSChart, gVariable.ALL_DATA_IN_DATABASE);
        ...
        private void Column_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (aTimer != null)
