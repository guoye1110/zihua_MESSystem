...
//            Console.WriteLine(DateTime.Now.ToString() + "11111c");
        ...
        void triggerAnAlarm(int chartType, int alarmCategory, int whichQulityItem, int startingIDInTable)
        {
            try
            {
                ...
                {
                    alarmTableStructImpl.errorDesc = "在平均值 (XBAR) 图中" + gVariable.errSPCDescList[alarmCategory - gVariable.ALARM_CATEGORY_SPC_DATA_START];
                    alarmTableStructImpl.type = gVariable.ALARM_TYPE_QUALITY_DATA;
                }
                else if (chartType == TYPE_S_CHART)
                {
                    alarmTableStructImpl.errorDesc = "在方差 (s) 图中" + gVariable.errSPCDescList[alarmCategory - gVariable.ALARM_CATEGORY_SPC_DATA_START];
                    alarmTableStructImpl.type = gVariable.ALARM_TYPE_QUALITY_DATA;
                }
                ...
                {
                    alarmTableStructImpl.errorDesc = "在样本缺陷数 (C) 图中" + gVariable.errSPCDescList[alarmCategory - gVariable.ALARM_CATEGORY_SPC_DATA_START];
                    alarmTableStructImpl.type = gVariable.ALARM_TYPE_QUALITY_DATA;
                }

                alarmTableStructImpl.alarmFailureCode = DateTime.Now.ToString("yyMMddHHmmss") + "_" + (gVariable.andonAlarmIndex + 1);
                alarmTableStructImpl.dispatchCode = gVariable.productTaskSheet[boardIndex].dispatchCode;
                alarmTableStructImpl.feedBinID = "";
                alarmTableStructImpl.machineName = gVariable.machineNameArrayDatabase[boardIndex];
