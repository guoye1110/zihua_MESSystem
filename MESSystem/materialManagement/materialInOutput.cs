...
{
    public partial class materialInOutput : Form
    {
        ...
        public static materialInOutput materialInOutputClass = null; //it is used to reference this windows

        int machineSelected;
        int workerIDSelected;
        ...
        private void initializeVariables()
        {
            ...
            label1.Text = gVariable.enterpriseTitle + "原料仓库出库与回收历史查询";
        ...
        private void materialManagement_Load(object sender, EventArgs e)
        {
            ...
            {
                comboBox2.Items.Add(workerNameList[i]);
        
            }
            comboBox2.SelectedIndex = workerIDSelected;
        ...
        private void displayMaterialList()
        {
            ...
            //int nameIndex;
            string name;
            int[] materialLenArray = { 1, 45, 110, 100, 100, 120, 100, 100, 80, 100, 100};
            string[] materialColumnArray = 
            {
                "", "序号", "物料编码", "物料批号", "出入库方向", "出入库时间", "出入库数量", "目标设备", "料仓编号", "配送员工", "库存余量", 
            };
            ...
            try
            {
                ...
                if (materialListArray != null)
                {
                    ...
                    for (i = 0; i < materialListArray.GetLength(0); i++)
                    {
                        ...
                        j = 0;
                        OptionItem.SubItems.Add(materialListArray[i, j++]);
                        OptionItem.SubItems.Add(materialListArray[i, j++]);
                        OptionItem.SubItems.Add(materialListArray[i, j++]);
                        if (materialListArray[i, j] == "1")
                        {
                            OptionItem.BackColor = Color.LightGreen;
                            OptionItem.SubItems.Add("物料出库");
                        }
                        else
                        {
                            OptionItem.BackColor = Color.LightYellow;
                            OptionItem.SubItems.Add("余料入库");
                        }
                        j++;
                        OptionItem.SubItems.Add(materialListArray[i, j++]);
                        OptionItem.SubItems.Add(materialListArray[i, j++] + "kg");
                        OptionItem.SubItems.Add(gVariable.machineNameArrayDatabase[Convert.ToInt32(materialListArray[i, j++])]);
                        OptionItem.SubItems.Add(materialListArray[i, j++]);
                        if(toolClass.isDigitalNum(materialListArray[i, j]) == 1)
                        {
                            name = toolClass.getNameByIDAndIDByName(null, materialListArray[i, j]);
                        }
                        else
                        {
                            name = "未知";
                        ...
                        OptionItem.SubItems.Add(materialInventoryList[i, 8]);
        ...
        void getMaterialListByCondition()
        {
            ...
            tableName = gVariable.materialDeliveryTableName;

            sthUpdated = 0;
            str1 = null;
            if (machineSelected != 0)
            {
                str1 = " targetMachine = '" + machineList[machineSelected] + "'";

                sthUpdated = 1;
            }

            str2 = null;
            if (workerIDSelected != 0)
            {
                if (sthUpdated == 0)
                    str2 = " deliveryWorker = '" + workerIDList[workerIDSelected] + "'";
                else
                    str2 = " and deliveryWorker = '" + workerIDList[workerIDSelected] + "'";
        ...
        {
            workerIDSelected = comboBox2.SelectedIndex;
        }
