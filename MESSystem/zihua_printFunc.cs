				...
				private int HandlePackingProcess(int communicationType, byte[] onePacket, int packetLen)
                {
                	...
                	if (communicationType == COMMUNICATION_TYPE_SLIT_PROCESS_PACKAGE_BARCODE_UPLOAD){
                        ...
                        try
                        {
                            ...
                            st_packing.productCode = strInputArray[4];
							st_packing.batchNum = strInputArray[5];
							st_packing.rollNumber = Convert.ToUInt16(strInputArray[6]);
                            st_packing.totalWeight = Convert.ToSingle(strInputArray[7]);
                            st_packing.totalLength = Convert.ToSingle(strInputArray[8]);
                            st_packing.uploadTime = System.DateTime.Now.ToString();
                            st_packing.machineID = (m_machineIDForPrint - PACKING_PROCESS_PC1 + gVariable.packingProcess[0]).ToString();
							st_packing.rollList = strInputArray[9];
                            //st_packing.smallRoll = strInputArray[6 + i];
                            /*for (int i = 0; i < st_packing.rollNumber; i++)
                            {
                                st_packing.smallRollList = strInputArray[6 + i] + ";";
                            }*/
                            //for (int i = 0; i < st_packing.rollNumber; i++)
                            //{
                            //    st_packing.smallRoll = strInputArray[6 + i] + ";";
					...
					if (communicationType == COMMUNICATION_TYPE_PACKING_PROCESS_PACKAGE_BARCODE_UPLOAD) {
						...
						//返回：<打包条码>;<铲板号>;<订单号>;<原材料代码>;<产品代号>;<产品批号>;<卷数>;<重量>;<长度>;<小卷条码信息>
						...
						if (pack!=null) {
							...
							str += pack.Value.batchNum+";"+pack.Value.rollNumber+";"+pack.Value.totalWeight+";"+pack.Value.totalLength+";"+pack.Value.rollList;
							m_ClientThread.sendStringToClient(str, communicationType);
							return -1;
						}
						return 0;
