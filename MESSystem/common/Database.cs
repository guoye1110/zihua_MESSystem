...
namespace MESSystem.common
{
    public partial class mySQLClass
    {
        ...
        const string strDispatchList2 = " value(@id, @machineID, @dispatchCode, @planTime1, @planTime2, @productCode, @productName, @operatorName, @forcastNum, @receiveNum, @qualifyNum, @unqualifyNum, @processName, " +
                                        "@startTime, @completeTime, @prepareTimePoint, @status, @waitForCheck, @wastedOutput, @workTeam, @serialNumber, @reportor, @workShop, @workShift, " +
        ...
        */
        ...
        public static gVariable.dispatchSheetStruct getDispatchByCommand(string databaseName, string tableName, string commandText)
        {
            MySqlDataReader myReader;
            ...
            try
            {
                ...
                myCommand.CommandText = "select * from `" + tableName.ToLower() + "`" + commandText;
                myReader = myCommand.ExecuteReader();
                if (myReader.Read())
                {
                    ...
                    if (!myReader.IsDBNull(SERIAL_NO_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.serialNumber = myReader.GetString(SERIAL_NO_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(REPORTOR_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.reportor = myReader.GetString(REPORTOR_IN_DISPATCHLIST_DATABASE);
                    ...
                    if (!myReader.IsDBNull(PRODUCT_COLOR_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.productColor = myReader.GetString(PRODUCT_COLOR_IN_DISPATCHLIST_DATABASE);
            ...
            {
                Console.WriteLine("database " + databaseName + "table:" + tableName + "getDispatchByComand() failed for " + commandText + "! " + ex);
            }
            ...
            try
            {
                if (myBoardIndex >= 0)
                {
                    ...
                    else //if(tableName == gVariable.productTaskListTableName))
                    {
                        ...
                        reportor = gVariable.productTaskSheet[myBoardIndex].reportor;
                    ...
                    updateString = "update `" + tableName + "` set receiveNum = " + received + ", qualifyNum = " + qualified + ", unqualifyNum = " + unqualified + ", wastedOutput = " + usedTime + ", reportor = '" + reportor +
