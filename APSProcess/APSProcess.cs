...
namespace MESSystem.APSDLL
{
    public partial class APSProcess
    {
        ...
        const int PRODUCT_BATCH_MAX_OUTPUT_NUM = 200000;
        ...
        static int APS_MACHINE_NUM = gVariable.machineNameArray.Length;
        ...
        private int startAPSAction(int APSPlanDirection, int salesOrderID, int APSStartTimeStamp, int APSEndTimeStamp, int[] assignedMachineByUserArray)
        {
            ...
            string[] productBatchString = new string[MAX_NUM_MACHINE_TYPE]; //产品批次号
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    for (i = 0; i < MAX_NUM_MACHINE_TYPE; i++)
                    {
                        ...
                        if (productNumInCurrentBatch[i] < productNumForOneBatch)
                        {
                            //keep production in this batch
                            productBatchIndex = getCurrentProductBatchIndex(monthIndex, machineIndexForAllType[i] + 1);
                            productBatchString[i] = DateTime.Now.Date.ToString("yyMM") + (machineIndexForAllType[i] + 1) + productBatchIndex.ToString().PadLeft(2, '0');
                        }
                        else if (productNumInCurrentBatch[i] > productNumForOneBatch && productNumInCurrentBatch[i] < PRODUCT_BATCH_MAX_OUTPUT_NUM)
                        {
                            ...
                            monthIndex = getMonthIndexForDispatchBatch(planTime1);
                            productBatchIndex = getCurrentProductBatchIndex(monthIndex, machineIndexForAllType[i] + 1);
                            setCurrentProductBatchIndex(monthIndex, machineIndexForAllType[i] + 1, productBatchIndex + 1);
                        }
                        else //if(productNumInCurrentBatch[i] > PRODUCT_BATCH_MAX_OUTPUT_NUM)
                        {
                            //product batch number increase by one, need to get new month value, maybe it's now new month 
                            planTime1 = GetTimeFromInt(castStartTimeStamp).ToString("yyyy-MM-dd HH:mm");
                            monthIndex = getMonthIndexForDispatchBatch(planTime1);
                            productBatchIndex = getCurrentProductBatchIndex(monthIndex, machineIndexForAllType[i] + 1) + 1;
                            setCurrentProductBatchIndex(monthIndex, machineIndexForAllType[i] + 1, productBatchIndex);
        ...
        private int getDispatchFromSalesOrder(int[] outNumForThisDispatch, string[] productBatchString)
        {
            ...
            if (timeV < 8)
            {
                ...
                timeStr = planTime1.Remove(0, 11).Remove(2, 1) + "2";
            }
            else if(timeV < 20)
            {
                workShift = "日班";
                timeStampDelta = 3600 * (20 - timeV);
                timeStr = planTime1.Remove(0, 11).Remove(2, 1) + "1";
            }
            else if (timeV < 24)
            {
                workShift = "夜班";
            ...
            try
            {
                ...
                if (outNumForThisDispatch[MACHINE_TYPE_CAST] > 0)
                {
                    ...
                    //cast process
                    dispatchSheet.machineID = (castMachineIndex + 1).ToString();
                    dispatchSheet.dispatchCode = productBatchString[0] + timeStr + "L" + (castMachineIndex + 1);
                ...
                if (printMachineIndex != -1 && outNumForThisDispatch[MACHINE_TYPE_PRINT] > 0)
                {
                    ...
                    dispatchSheet.dispatchCode = productBatchString[1] + timeStr + "Y" + (printMachineIndex - gVariable.printingProcess[0] + 1);
                ...
                if (outNumForThisDispatch[MACHINE_TYPE_SLIT] > 0)
                {
                    ...
                    dispatchSheet.dispatchCode = productBatchString[2] + timeStr + "F" + (printMachineIndex - gVariable.printingProcess[0] + 1);
        ...
        void generateMaterialList(string databaseName, string tableName, string dispatchCode, string machineID, int outputNum, string BOMCode)
        {
            ...
            try
            {
                ...
                materialSheet.machineName = gVariable.machineNameArray[Convert.ToInt32(machineID) - 1];
                ...
                for (i = 0; i < materialSheet.numberOfTypes; i++)
                {
                    ...
                    materialSheet.materialRequired[i] = BOMImpl.materialQuantity[i] * outputNum * (1000 / 100);  //material requirement by kilogram
