...
namespace MESSystem.common
{
	public class reusematerialDB : mySQLClass
	{
        ...
        private const int REBUILD_DATE_INDEX = 2;
        private const int BOM_CODE_INDEX = 3;
		private const int BARCODE_FOR_REUSE_INDEX =4;
        private const int ORIGINAL_NUM_INDEX = 5;
        private const int REBUILD_NUM_INDEX = 6;
        private const int WORKER_ID_INDEX = 7;
		private const int BARCODE1_INDEX = 8;
		private const int BARCODE2_INDEX = 9;
		private const int BARCODE3_INDEX = 10;
		private const int BARCODE4_INDEX = 11;
		private const int BARCODE5_INDEX = 12;
		private const int BARCODE6_INDEX = 13;
		private const int BARCODE7_INDEX = 14;
		private const int BARCODE8_INDEX = 15;
		private const int BARCODE9_INDEX = 16;
		private const int BARCODE10_INDEX = 17;
		private const int TOTAL_DATAGRAM_NUM = BARCODE10_INDEX;

		private const string c_dbName = "globaldatabase";
        private const string c_TableName = "reusematerialList";
		...
		public struct reusematerial_t{
            ...
            public float? originalNum;
            public float? rebuildNum;
		...
		public string Serialize(reusematerial_t st)
		{
            ...
            str += st.machineID     + ";" + st.rebuildDate      + ";" + st.BOMCode          + ";" + st.barcodeForReuse  + ";" + st.originalNum  + ";";
			str += st.rebuildNum    + ";" + st.workerID		    + ";" + st.barCode1 		+ ";" + st.barCode2 		+ ";";
		...
		public reusematerial_t? Deserialize(string strInput)
		{
            ...
            if (input[ORIGINAL_NUM_INDEX] != "") st.originalNum = Convert.ToUInt32(input[ORIGINAL_NUM_INDEX], 10);
