...
namespace MESSystem.APS_UI
{
    public partial class APSExhibit : Form
    {
        ...
        private void resizeForScreen()
        {
            ...
            Label[] labelArray = { label1, label2, label3, label4, label14, label6, label7, label8, label9, label10, label11, label12 };
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            int[] contentArray_Y = { 58, 60, 64, 64, 74, 86};
            int x = 3;
            int y;
            int i, j, k;
            int w;
            ...
            string databaseName;
            string[,] tableArray;
            string [] salesOrderArray = new string[SALES_ORDER_NUM_MAX];
            ...
            try
            {
                ...
                numOfMachines = gVariable.machineNameArray.Length;
                ...
                for (i = 0; i < numOfMachines + 1; i++)
                {
                    ...
                    e.Graphics.DrawString(gVariable.machineNameArray[i], myFont, colorBlackBrush, x + offsetX * 2, y + titleH * 2 + i * deltaY + offsetY * 2);
                }
                ...
                for (i = 0; i < numOfMachines + 1; i++)
                {
                    ...
                    {
                        for(k = 0; k < salesOrderNum; k++)
                        {
                            if (tableArray[j, mySQLClass.SALES_ORDER_INDEX] == salesOrderArray[k])
                                break;
                        }

                        //a new sales order, put it in sales order array
                        if(k >= salesOrderNum)
                        {
                            salesOrderArray[salesOrderNum++] = tableArray[j, mySQLClass.SALES_ORDER_INDEX];
                        }

                        currentColor = k;
                        if (currentColor >= SALES_ORDER_NUM_MAX)
                            currentColor = SALES_ORDER_NUM_MAX - 1;

                        startTime = Convert.ToInt32(tableArray[j, mySQLClass.START_TIME_STAMP_INDEX]);
                        ...
                        e.Graphics.FillRectangle(brushArray[currentColor], xx, yy, ww, hh);
                        e.Graphics.DrawRectangle(blackLine, xx, yy, ww, hh);

                        k = mySQLClass.SALES_ORDER_INDEX;
                        ...
                        dispatchBlockImpl[dispatchBlockIndex].machineName = gVariable.machineNameArray[i];
