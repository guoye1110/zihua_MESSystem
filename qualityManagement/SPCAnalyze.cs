        ...
        private int setComboBoxes(int whereFrom)
        {
            ...
            try
            {
                ...
                else  //from quality management
                {
                    ...
                    {
                        comboBox1.Items.Add(gVariable.machineNameArray[i]);
                    }
                    ...
                    dataNum = toolClass.getQualityDataItemByDispatch(databaseName, gVariable.qualityListTableName, dispatchListArray[dispatchSelected], qualityDataItemArray);
                    for (i = 0; i < dataNum; i++)
                    {
...
//                        numOfPointNeedForC_Chart
                ...
                {
                    dispatchCode = gVariable.dispatchSheet[boardIndex].dispatchCode;
                    ...
                    totalDataNum = mySQLClass.getRecordNumInTable(databaseName, gVariable.dispatchSheet[boardIndex].dispatchCode + gVariable.qualityTableNameAppendex);
                ...
                if (SPCFromAlarmOrQualityManagement == gVariable.FROM_ALARM_DISPLAY_FUNC)
                {
                    ...
                    else if (category >= gVariable.ALARM_CATEGORY_SPC_DATA_START && category <= gVariable.ALARM_CATEGORY_SPC_DATA_LOCATE_APART)
                    {
                        strArray = alarmTableStructImpl.errorDesc.Split(':');
                        errorItem = strArray[0];
                        groupBox1.Text = "工单" + dispatchCode + " " + errorItem; // +" " + gConstText.errSPCDescList[category - gVariable.ALARM_CATEGORY_SPC_DATA_START];
                    }
                    else
                    {
                        strArray = alarmTableStructImpl.errorDesc.Split(':');
                ...
                else //if (SPCFromAlarmOrQualityManagement == gVariable.FROM_QUALITY_MANAGEMENT_FUNC)
                {
                    ...
                    groupBox1.Text = gVariable.machineNameArray[i] + " 订单号:";
