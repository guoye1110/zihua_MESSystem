...
namespace MESSystem.common
{
    public partial class mySQLClass
    {
        ...
        public const int SALESORDER_BATCH_CODE_IN_DISPATCHLIST_DATABASE = 39;
        ...
        public const int PLAN_MAINTENANCE_ID_INDEX = 1;
        public const int PLAN_SALES_ORDER_INDEX = 2;
        public const int PLAN_DISPATCH_CODE_INDEX = 3;
        public const int PLAN_START_TIME_INDEX = 4;
        public const int PLAN_START_TIME_STAMP_INDEX = 5;
        public const int PLAN_KEEP_DURATION_INDEX = 6;
        public const int PLAN_END_TIME_INDEX = 7;
        public const int PLAN_END_TIME_STAMP_INDEX = 8;
        ...
        const string strDispatchList1 = "(id int(1) AUTO_INCREMENT primary key, machineID varchar(40), dispatchCode varchar(40), planTime1 varchar(20), planTime2 varchar(20), productCode varchar(40), " +
                                         ...
                                         "startTime varchar(20), completeTime varchar(20), prepareTimePoint varchar(20), status int(1), waitForCheck int(1), wastedOutput int(1), workTeam varchar(40), " +
                                         "batchNum2 int(1), batchNum3 varchar(14), workShop varchar(40), workShift varchar(20), salesOrderCode varchar(40), BOMCode varchar(40)) ENGINE = MYISAM CHARSET=utf8";
        const string strDispatchList2 = " value(@id, @machineID, @dispatchCode, @planTime1, @planTime2, @productCode, @productName, @operatorName, @forcastNum, @receiveNum, @qualifyNum, @unqualifyNum, @processName, " +
                                        "@startTime, @completeTime, @prepareTimePoint, @status, @waitForCheck, @wastedOutput, @workTeam, @batchNum2, @batchNum3, @workShop, @workShift, " +
        ...
        const string strMachineStatus1 = "(id int(1) AUTO_INCREMENT primary key, dispatchCode varchar(40), totalWorkingTime int(1), collectedNumber int(1), productBeat int(1), workingTime int(1), " +
                                          ...
                                          "waitForCheck int(1), wastedOutput int(1)) ENGINE = MYISAM CHARSET=utf8";
        const string strMachineStatus2 = " value(@id, @dispatchCode, @totalWorkingTime, @collectedNumber, @productBeat, @workingTime, @prepareTime, @standbyTime, @power, @powerConsumed, @revolution, " +
                                         "@maintenancePeriod, @lastMaintenance, @waitForCheck, @wastedOutput)";
        ...
        static string insertStringMaterialRequired;
        ...
        public static string[] infoDatabaseName2 = 
        {
            ...
            gVariable.basicInfoDatabaseName, gVariable.basicInfoDatabaseName, gVariable.basicInfoDatabaseName, gVariable.basicInfoDatabaseName, gVariable.basicInfoDatabaseName, gVariable.basicInfoDatabaseName, 
            gVariable.basicInfoDatabaseName, gVariable.basicInfoDatabaseName, gVariable.basicInfoDatabaseName, gVariable.basicInfoDatabaseName, gVariable.basicInfoDatabaseName, gVariable.basicInfoDatabaseName, 
            gVariable.basicInfoDatabaseName, gVariable.basicInfoDatabaseName, gVariable.basicInfoDatabaseName, gVariable.basicInfoDatabaseName, 
        };
        ...
        { 
            ...
            gVariable.warehouseListTableName, gVariable.packingTableName, gVariable.substitutesTableName, gVariable.machineCapabilityTableName,
        };
        ...
        public static string[] infoTableFileName2 = 
        {
            ...
            gVariable.warehouseListFileName, gVariable.packingFileName, gVariable.substitutesFileName, gVariable.machineCapabilityFileName,
        };
        ...
        public mySQLClass()
        {
            ...
            //the speed of reading excel file is very slow, so we read them here athe beginning, and put the result in global parameter
            getDispatchInsertString();
            getMaterialInsertString();
            getWorkingPlanInsertString();
            getmaterialRequiredInsertString();
        }
        ...
        */
        ...
        public static void buildBasicDatabase()
        {
            ...
            try
            {
                ...
                for (i = 0; i < gVariable.maxMachineNum + 1; i++)
                {
                    ...
                    createDataTableFromExcel(dname, gVariable.productTaskListTableName, gVariable.dispatchListFileName, LEN_40, 0);
                    ...
                    createDataTableFromExcel(dname, gVariable.materialRequirementTableName, gVariable.materialRequirementFileName, LEN_40, 0);
                    createDataTableFromExcel(dname, gVariable.materialTaskListTableName, gVariable.materialListFileName, LEN_40, 0);
                ...
                {
                    if (i == 0 || i == 2 || i == 10)   //alarm/material/finalPacking table need no predefined data, while sales order need predefined data from excel file for testing
        ...
        //only one dispatch will be returned 
        public static gVariable.dispatchSheetStruct getDispatchByCommand(string databaseName, string commandText)
        {
            MySqlDataReader myReader;
            gVariable.dispatchSheetStruct dispatchSheet;

            dispatchSheet = new gVariable.dispatchSheetStruct();
            try
            {
                MySqlConnection myConnection = new MySqlConnection("database = " + databaseName + "; " + connectionString);
                myConnection.Open();
                ...
                if (myReader.Read())
                {
                    ...
                    if (!myReader.IsDBNull(MACHINE_ID_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.machineID = myReader.GetString(MACHINE_ID_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(DISPATCH_CODE_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.dispatchCode = myReader.GetString(DISPATCH_CODE_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(PLANNED_STARTTIME_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.planTime1 = myReader.GetString(PLANNED_STARTTIME_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(PLANNED_COMPLETETIME_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.planTime2 = myReader.GetString(PLANNED_COMPLETETIME_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(PRODUCT_CODE_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.productCode = myReader.GetString(PRODUCT_CODE_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(PRODUCT_NAME_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.productName = myReader.GetString(PRODUCT_NAME_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(OPERATOR_NAME_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.operatorName = myReader.GetString(OPERATOR_NAME_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(FORCAST_NUM_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.plannedNumber = Convert.ToInt32(myReader.GetString(FORCAST_NUM_IN_DISPATCHLIST_DATABASE));

                    if (!myReader.IsDBNull(RECEIVE_NUM_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.outputNumber = Convert.ToInt32(myReader.GetString(RECEIVE_NUM_IN_DISPATCHLIST_DATABASE));

                    if (!myReader.IsDBNull(QUALIFIED_NUM_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.qualifiedNumber = Convert.ToInt32(myReader.GetString(QUALIFIED_NUM_IN_DISPATCHLIST_DATABASE));

                    if (!myReader.IsDBNull(UNQUALIFIED_NUM_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.unqualifiedNumber = Convert.ToInt32(myReader.GetString(UNQUALIFIED_NUM_IN_DISPATCHLIST_DATABASE));

                    if (!myReader.IsDBNull(PROCESS_NAME_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.processName = myReader.GetString(PROCESS_NAME_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(START_TIME_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.realStartTime = myReader.GetString(START_TIME_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(COMPLETE_TIME_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.realFinishTime = myReader.GetString(COMPLETE_TIME_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(PREPARE_TIME_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.prepareTimePoint = myReader.GetString(PREPARE_TIME_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(STATUS_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.status = Convert.ToInt32(myReader.GetString(STATUS_IN_DISPATCHLIST_DATABASE));

                    if (!myReader.IsDBNull(EQUIPMENT_LIFETIME_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.waitForCheck = Convert.ToInt32(myReader.GetString(EQUIPMENT_LIFETIME_IN_DISPATCHLIST_DATABASE));

                    if (!myReader.IsDBNull(EQUIPMENT_USEDTIME_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.wastedOutput = Convert.ToInt32(myReader.GetString(EQUIPMENT_USEDTIME_IN_DISPATCHLIST_DATABASE));

                    if (!myReader.IsDBNull(OUTPUT_RATIO_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.workTeam = myReader.GetString(OUTPUT_RATIO_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(SERIAL_NO_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.batchNum2 = myReader.GetString(SERIAL_NO_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(REPORTOR_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.batchNum3 = myReader.GetString(REPORTOR_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(WORKSHOP_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.workshop = myReader.GetString(WORKSHOP_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(WORK_SHIFT_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.workshift = myReader.GetString(WORK_SHIFT_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(SALESORDER_CODE_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.salesOrderCode = myReader.GetString(SALESORDER_CODE_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(BOM_CODE_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.BOMCode = myReader.GetString(BOM_CODE_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(CUSTOMER_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.customer = myReader.GetString(CUSTOMER_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(MULTI_PRODUCT_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.multiProduct = myReader.GetString(MULTI_PRODUCT_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(PRODUCT_CODE2_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.productCode2 = myReader.GetString(PRODUCT_CODE2_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(PRODUCT_CODE3_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.productCode3 = myReader.GetString(PRODUCT_CODE3_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(OPERATOR2_NAME_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.operatorName2 = myReader.GetString(OPERATOR2_NAME_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(OPERATOR3_NAME_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.operatorName3 = myReader.GetString(OPERATOR3_NAME_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(BATCH_NUMBER_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.batchNum = myReader.GetString(BATCH_NUMBER_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(PRODUCT_COLOR_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.batchNum1 = myReader.GetString(PRODUCT_COLOR_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(RAW_MATERIAL_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.rawMateialcode = myReader.GetString(RAW_MATERIAL_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(PRODUCT_LENGTH_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.productLength = myReader.GetString(PRODUCT_LENGTH_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(PRODUCT_DIAMETER_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.productDiameter = myReader.GetString(PRODUCT_DIAMETER_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(PRODUCT_WEIGHT_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.productWeight = myReader.GetString(PRODUCT_WEIGHT_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(SLIT_WIDTH_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.slitWidth = myReader.GetString(SLIT_WIDTH_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(SALESORDER_BATCH_CODE_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.salesOrderBatchCode = myReader.GetString(SALESORDER_BATCH_CODE_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(PRODUCT_CODE4_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.productCode4 = myReader.GetString(PRODUCT_CODE4_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(OPERATOR4_NAME_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.operatorName4 = myReader.GetString(OPERATOR4_NAME_IN_DISPATCHLIST_DATABASE);

                    if (!myReader.IsDBNull(NOTES_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.notes = myReader.GetString(NOTES_IN_DISPATCHLIST_DATABASE);
                    
                    if (!myReader.IsDBNull(COMMENTS_IN_DISPATCHLIST_DATABASE))
                        dispatchSheet.comments = myReader.GetString(COMMENTS_IN_DISPATCHLIST_DATABASE);
                }
                else
                {
                    dispatchSheet.batchNum = "0";
                }
                myReader.Close();
                myConnection.Close();
            ...
            {
                Console.WriteLine("database " + databaseName + "table: getDispatchByComand() failed for " + commandText + "! " + ex);
            }
                ...
                if (myReader != null)
                {
                    while (true)
                    {
                        ...
                        else
                        {
                            ...
                            if (!myReader.IsDBNull(EQUIPMENT_LIFETIME_IN_DISPATCHLIST_DATABASE))
                                dispatchList[index].waitForCheck = Convert.ToInt32(myReader.GetString(EQUIPMENT_LIFETIME_IN_DISPATCHLIST_DATABASE));

                            if (!myReader.IsDBNull(EQUIPMENT_USEDTIME_IN_DISPATCHLIST_DATABASE))
                                dispatchList[index].wastedOutput = Convert.ToInt32(myReader.GetString(EQUIPMENT_USEDTIME_IN_DISPATCHLIST_DATABASE));

                            if (!myReader.IsDBNull(OUTPUT_RATIO_IN_DISPATCHLIST_DATABASE))
                                dispatchList[index].workTeam = myReader.GetString(OUTPUT_RATIO_IN_DISPATCHLIST_DATABASE);

                            if (!myReader.IsDBNull(SERIAL_NO_IN_DISPATCHLIST_DATABASE))
                                dispatchList[index].batchNum2 = myReader.GetString(SERIAL_NO_IN_DISPATCHLIST_DATABASE);

                            if (!myReader.IsDBNull(REPORTOR_IN_DISPATCHLIST_DATABASE))
                                dispatchList[index].batchNum3 = myReader.GetString(REPORTOR_IN_DISPATCHLIST_DATABASE);
                            ...
                            if (!myReader.IsDBNull(PRODUCT_COLOR_IN_DISPATCHLIST_DATABASE))
                                dispatchList[index].batchNum1 = myReader.GetString(PRODUCT_COLOR_IN_DISPATCHLIST_DATABASE);
                            ...
                            if (!myReader.IsDBNull(SALESORDER_BATCH_CODE_IN_DISPATCHLIST_DATABASE))
                                dispatchList[index].salesOrderBatchCode = myReader.GetString(SALESORDER_BATCH_CODE_IN_DISPATCHLIST_DATABASE);
        ...
        */
        ...
        public static void updateDispatchTable(String databaseName, String tableName, int myBoardIndex, int status, string dispatchCodeV)
        {
            ...
            try
            {
                if (myBoardIndex >= 0)
                {
                    if (tableName == gVariable.dispatchListTableName)
                    {
                        ...
                        batchNum3 = gVariable.dispatchSheet[myBoardIndex].batchNum3;
                        ...
                        usedTime = gVariable.dispatchSheet[myBoardIndex].wastedOutput;
                    }
                    else //if(tableName == gVariable.productTaskListTableName))
                    {
                        dispatchCode = gVariable.productTaskSheet[myBoardIndex].dispatchCode;
                        received = gVariable.productTaskSheet[myBoardIndex].outputNumber;
                        qualified = gVariable.productTaskSheet[myBoardIndex].qualifiedNumber;
                        unqualified = gVariable.productTaskSheet[myBoardIndex].unqualifiedNumber;
                        batchNum3 = gVariable.productTaskSheet[myBoardIndex].batchNum3;
                        prepareTime = gVariable.productTaskSheet[myBoardIndex].prepareTimePoint;
                        start = gVariable.productTaskSheet[myBoardIndex].realStartTime;
                        finish = gVariable.productTaskSheet[myBoardIndex].realFinishTime;
                        usedTime = gVariable.productTaskSheet[myBoardIndex].wastedOutput;
                    }
                    updateString = "update `" + tableName + "` set receiveNum = " + received + ", qualifyNum = " + qualified + ", unqualifyNum = " + unqualified + ", wastedOutput = " + usedTime + ", batchNum3 = '" + batchNum3 +
        ...
        public static int writeDataToProductBatchTable(string tableName, string batchOrderFileName, gVariable.productBatchStruct batchOrderImpl)
        {
            ...
            try
            {
                ...
                myCommand.Parameters.AddWithValue(itemName[index++], batchOrderImpl.productCode2);
                myCommand.Parameters.AddWithValue(itemName[index++], batchOrderImpl.customer2);
                myCommand.Parameters.AddWithValue(itemName[index++], batchOrderImpl.productCode3);
                myCommand.Parameters.AddWithValue(itemName[index++], batchOrderImpl.customer3);
                myCommand.Parameters.AddWithValue(itemName[index++], batchOrderImpl.productCode4);
                myCommand.Parameters.AddWithValue(itemName[index++], batchOrderImpl.customer4);
        ...
        static void getmaterialRequiredInsertString()
        {
            getDatabaseInsertStringFromExcel(ref insertStringMaterialRequired, gVariable.materialRequirementFileName);
        }
        //return 1 means dispatch already exist
        //       0 means dispatch not exist, now has been written to table successfully
        //      -1 exception occurred
        public static int writeDataToDispatchListTable(String databaseName, String tableName, gVariable.dispatchSheetStruct dispatchImpl)
        {
            ...
            try
            {
                ...
                myCommand.Parameters.AddWithValue(itemName[index++], dispatchImpl.waitForCheck);
                myCommand.Parameters.AddWithValue(itemName[index++], dispatchImpl.wastedOutput);
                myCommand.Parameters.AddWithValue(itemName[index++], dispatchImpl.workTeam);
                myCommand.Parameters.AddWithValue(itemName[index++], dispatchImpl.batchNum2);
                myCommand.Parameters.AddWithValue(itemName[index++], dispatchImpl.batchNum3);
                ...
                myCommand.Parameters.AddWithValue(itemName[index++], dispatchImpl.batchNum1);
        ...
        public static void writeDataToMachineStatusTable(String databaseName, String tableName, int myBoardIndex)
        {
            ...
            try
            {
                ...
                myCommand.Parameters.AddWithValue(itemName[index++], gVariable.machineStatus[myBoardIndex].waitForCheck);
                myCommand.Parameters.AddWithValue(itemName[index++], gVariable.machineStatus[myBoardIndex].wastedOutput);
        ...
        public static void writeDataToMaterialListTable(string databaseName, string tableName, gVariable.materialListStruct materialListImpl)
        {
            ...
            try
            {
                ...
                for (i = 0; i < MAX_MATERIAL_TYPE_IN_BOM; i++)
                {
                    ...
                    myCommand.Parameters.AddWithValue(itemName[index++], materialListImpl.previousLeft[i]);
                    myCommand.Parameters.AddWithValue(itemName[index++], materialListImpl.currentLeft[i]);
                    myCommand.Parameters.AddWithValue(itemName[index++], materialListImpl.currentlyUsed[i]);
                ...
                myCommand.Parameters.AddWithValue(itemName[index++], materialListImpl.batchNum);
                myCommand.Parameters.AddWithValue(itemName[index++], materialListImpl.materialStatus);
                myCommand.ExecuteNonQuery();
                myConnection.Close();
            }
            catch (Exception ex)
        ...
        public static void writeDataToMaterialRequirementTable(string databaseName, string tableName, gVariable.materialRequirementStruct materialRequirementImpl)
        {
            int i;
            int index;
            //int itemNum;
            ...
            try
            {
                ...
                myCommand.CommandText = "insert into `" + tableName + "`" + insertStringMaterialRequired;

                index = 0;
                itemName = insertStringMaterialRequired.Remove(0, 11).Split(',', ')');

                myCommand.Parameters.AddWithValue("@id", 0);
                myCommand.Parameters.AddWithValue(itemName[index++], materialRequirementImpl.materialRequiresheetCode);
                myCommand.Parameters.AddWithValue(itemName[index++], materialRequirementImpl.salesOrderCode);
                myCommand.Parameters.AddWithValue(itemName[index++], materialRequirementImpl.salesOrderBatchCode);
                myCommand.Parameters.AddWithValue(itemName[index++], materialRequirementImpl.batchNum);
                myCommand.Parameters.AddWithValue(itemName[index++], materialRequirementImpl.status);
                myCommand.Parameters.AddWithValue(itemName[index++], materialRequirementImpl.machineID);
                myCommand.Parameters.AddWithValue(itemName[index++], materialRequirementImpl.numberOfTypes);

                for (i = 0; i < MAX_MATERIAL_TYPE_IN_BOM; i++)
                {
                    myCommand.Parameters.AddWithValue(itemName[index++], materialRequirementImpl.materialCode[i]);
                    myCommand.Parameters.AddWithValue(itemName[index++], materialRequirementImpl.materialRequired[i]);
                }
                myCommand.ExecuteNonQuery();
                myConnection.Close();
            }
        ...
        public static void writeFeedBinTable(string databaseName, string tableName, string fileName, string[] param)
        {
            int i;
            int itemNum;
            string insertString;
            ...
            try
            {
                itemNum = getDatabaseInsertStringFromExcel(ref insertString, fileName);
                if (itemNum < 0)
                {
                    Console.WriteLine("database " + databaseName + "table " + tableName + " write failed, excel read error!");
                    return;
                ...
                {
                    myCommand.Parameters.AddWithValue(itemName[i], param[i]);
                }
                myCommand.ExecuteNonQuery();
                myCommand.Parameters.Clear();
                myConnection.Close();
