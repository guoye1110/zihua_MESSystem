...
namespace MESSystem.dispatchManagement
{
    public partial class dispatchPublish : Form
    {
        ...
        void resizeScreen()
        {
            ...
            Button[] buttonArray = { button1, button2, button3, button4, button5, button6 };
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                for (i = 0; i < listView1.CheckedItems.Count; i++)
                {
                    ...
                    //update status of product batch to published
                    ...
                    //then update this product task to the status of published
                    commandText = "update `" + gVariable.globalProductTaskTableName + "` set status = '" + gVariable.MACHINE_STATUS_DISPATCH_PUBLISHED + "' where salesOrderBatchCode = '" + batchTableArray[index, 1] + "'";
                    mySQLClass.pureDatabaseNonQueryAction(gVariable.globalDatabaseName, commandText);

                    //copy this product task to database for the specific machine
                    commandText = "select * from `" + gVariable.globalProductTaskTableName + "` where salesOrderBatchCode = '" + batchTableArray[index, 1] + "'";
                    tableArray = mySQLClass.databaseCommonReading(gVariable.globalDatabaseName, commandText);
                    if (tableArray == null)
                    {
                        Console.WriteLine("failed to find product task with this code！-- only internal warning, not an exception.");
                        return;
                    }
                    for (j = 0; j < tableArray.GetLength(0); j++)
                    {
                        ...
                        commandText = "insert into " + databaseName + "." + gVariable.productTaskListTableName + "(" + columnsDispatch + ") select " + columnsDispatch + " from " + gVariable.globalDatabaseName + "." +
                                       gVariable.globalProductTaskTableName + " where dispatchCode = '" + tableArray[j, 2] + "'";
                        mySQLClass.pureDatabaseNonQueryAction(databaseName, commandText);
                    }

                    //copy related material tasks to database for the specific machine
                    commandText = "select * from `" + gVariable.globalMaterialTaskTableName + "` where salesOrderBatchCode = '" + batchTableArray[index, 1] + "'";
                    tableArray = mySQLClass.databaseCommonReading(gVariable.globalDatabaseName, commandText);
                    if (tableArray == null)
                    {
                        Console.WriteLine("failed to find material task with this code！-- only internal warning, not an exception.");
                        return;
                    }
                    for (j = 0; j < tableArray.GetLength(0); j++)
                    {
                        machineID = Convert.ToInt32(tableArray[j, 3]); // Convert.ToInt32(dispatchListArray[index].machineID);
                        databaseName = gVariable.DBHeadString + machineID.ToString().PadLeft(3, '0');
                        commandText = "insert into " + databaseName + "." + gVariable.materialTaskListTableName + "(" + columnsMaterial + ") select " + columnsMaterial + " from " + gVariable.globalDatabaseName + "." +
                                       gVariable.globalMaterialTaskTableName + " where dispatchCode = '" + tableArray[j, 2] + "'";
                        mySQLClass.pureDatabaseNonQueryAction(databaseName, commandText);
                    }

                    //dispatches generated from APS won't be used any more, and won't be sent to machine database, we will generate real dispatchyes when we get apply requirement from touchscreen
                    /*
                    //finally update this dispatch to the status published
                    ...
                    //copy all dispatches to database in the specific machine
                    commandText = "select * from `" + gVariable.globalDispatchTableName + "` where salesOrderBatchCode = '" + batchTableArray[index, 1] + "'";
                    tableArray = mySQLClass.databaseCommonReading(gVariable.globalDatabaseName, commandText);
                    if (tableArray == null)
                    {
                        Console.WriteLine("failed to find product task with this code 1！-- only internal warning, not an exception.");
                        return;
                    }
                    for (j = 0; j < tableArray.GetLength(0); j++)
                    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            str1 = dateTimePicker3.Value.ToString();
            str2 = dateTimePicker4.Value.ToString();

            if (str1 != null && str2 != null && str1.Length > 5 && str2.Length > 5)
            {
        ...
        void getBatchListByCondition()
        {
            ...
            */
            ...
            if (startDatePlanned != null)
            {
                if (sthUpdated == 0)
                {
                    str5 = " time1 >= '" + startDatePlanned + "' and time2 <= '" + endDatePlanned + "' 23:59";
                }
                else
                {
                    str5 = " and time1 >= '" + startDatePlanned + "' and time2 <= '" + endDatePlanned + "' 23:59";
                }
            ...
            if (batchTableArray == null)
            {
                Console.WriteLine("getBatchListByCondition failed！ -- only internal warning, not an exception.");
                return;
            }
        ...
        public void cancelSelectedDispatches()
        {
            ...
            try
            {
                ...
                for (i = 0; i < listView1.CheckedItems.Count; i++)
                {
                    ...
                    //first update this product task to the status published
                    commandText = "update `" + gVariable.globalProductTaskTableName + "` set status = '0' where salesOrderBatchCode = '" + batchTableArray[index, 1] + "'";
                    mySQLClass.pureDatabaseNonQueryAction(gVariable.globalDatabaseName, commandText);

                    //remove product tasks in database
                    commandText = "select * from `" + gVariable.globalProductTaskTableName + "` where salesOrderBatchCode = '" + batchTableArray[index, 1] + "'";
                    tableArray = mySQLClass.databaseCommonReading(gVariable.globalDatabaseName, commandText);
                    if (tableArray == null)
                    {
                        Console.WriteLine("failed to find dispatch with this code 2！-- only internal warning, not an exception.");
                        return;
                    }
                    ...
                    for (j = 0; j < tableArray.GetLength(0); j++)
                    {
                        ...
                        commandText = "delete from `" + gVariable.productTaskListTableName + "` where dispatchCode = '" + tableArray[j, 2] + "'";
                        mySQLClass.pureDatabaseNonQueryAction(databaseName, commandText);
                        //mySQLClass.redoIDIncreamentAfterRecordDeleted(databaseName, gVariable.productTaskListTableName);
                    }
                    ...
                    if (tableArray == null)
                    {
                        Console.WriteLine("failed to find dispatch with this code 3！-- only internal warning, not an exception.");
                        return;
                    }
        ...
        */
        ...
        private void button4_Click_2(object sender, EventArgs e)
        {
            ...
            for (i = 0; i < listView1.CheckedItems.Count; i++)
            {
                ...
                if (Convert.ToInt32(batchTableArray[index, 17]) != gVariable.SALES_ORDER_STATUS_CANCEL_APPLIED)
                {
                    MessageBox.Show("请确认所有选中的任务单都处于已取消状态，其他状态无法删除。", "提示信息", MessageBoxButtons.OK);
                    return;
                }
            }

            dr = MessageBox.Show("确认删除该任务单吗？", "需要确认", MessageBoxButtons.YesNo);
            if (dr == DialogResult.No)
            {
                return;
            }
            ...
            {
                index = listView1.CheckedItems[i].Index;
                //commandText = "delete `" + gVariable.productBatchTableName + "` set orderStatus = '" + gVariable.SALES_ORDER_STATUS_CANCELLED + "' where id = '" + batchTableArray[index, 0] + "'";
                //mySQLClass.pureDatabaseNonQueryAction(gVariable.globalDatabaseName, commandText);
            }
        }
    }
