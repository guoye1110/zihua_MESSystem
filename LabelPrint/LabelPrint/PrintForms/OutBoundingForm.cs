      ...
      //      UserInput.WeightPerBag = tb_WeightPerBag.Text;
        ...
        void serialDataReceived2(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            try
            {
                ...
                ToServer_material_out_upload(str);
            }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try {
                ...
                //close form here, then generate Ruku form, otherwise, serial port will have problem
                this.Close();
		...
		private int ToServer_startwork()
        {
			...
			if (data != null) {
				...
				for (int i=0;i< OutBoundingInputData.targets.Length;i++) {
					for (int j=0;j< MAX_LIAOCANG_NUM; j++) {
						m_materialCode[i,j] = start_work[i*14+j*2];
						m_materialRequired[i,j] = start_work[i*14+j*2+1];
					}
				}

                bStart = true;

                cb_TargetMachineNo.Text  = OutBoundingInputData.targets[0];
		...
		//返回值：		 0：	成功
		//			-1：通讯失败
		private int ToServer_material_out_upload(string str)
		{
			...
			if (m_connected) {
                tableArray = str.Split(';');
                uploadStr = tableArray[0] + ";" + UserInput.RawMaterialBatchNo + ";" + tableArray[2] + ";" + tableArray[3] + ";" + tableArray[4];

                send_buf = System.Text.Encoding.Default.GetBytes(uploadStr);
                /*
		...
		private void network_data_received(int communicationType, byte[] data_buf, int len)
		{
            ...
            try
            {
                if (communicationType == COMMUNICATION_TYPE_WAREHOUE_OUT_START)
                {
                    if (data_buf != null)
                    {
                        ...
                        //7台设备，每台7个料仓，一共49组数据，每组数据格式如下：物料代码;物料数量;
                        for (i = 0; i < OutBoundingInputData.targets.Length; i++)
                        {
                            for (j = 0; j < MAX_LIAOCANG_NUM; j++)
                            {
                                m_materialCode[i, j] = start_work[i * 14 + j * 2];
                                m_materialRequired[i, j] = start_work[i * 14 + j * 2 + 1];
                            }
                        ...
                        this.Invoke((EventHandler)(delegate
                        {
                            index = cb_TargetMachineNo.SelectedIndex;
                            cb_TargetMachineNo.Text = OutBoundingInputData.targets[index];

                            for (i = 0; i < MAX_LIAOCANG_NUM; i++)
                            {
                                cb_RawMaterialCodes[i].Text = m_materialCode[index, i];
                                tb_XuQiuWeights[i].Text = m_materialRequired[index, i];
                            }
                        }));
            ...
            {
                Console.WriteLine("network_data_received in outbound failed: " + ex);
            }
