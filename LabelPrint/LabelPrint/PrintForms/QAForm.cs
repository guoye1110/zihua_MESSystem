...
namespace LabelPrint
{
    public partial class QAForm : Form
    {
		...
		private void network_data_received(int communicationType, byte[] data_buf, int len)
		{
            ...
            {
                Console.WriteLine("network_data_received in QA form failed" + ex);
            }
        ...
        private void QAForm_Load(object sender, EventArgs e)
        {
            ...
            UserInput.QAMachineNo = SysSetting.CurSettingInfo.MachineNo;
        ...
        void serialDataReceived2(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            try
            {
                ...
                barcode = System.Text.Encoding.ASCII.GetString(serialDataBuf1, 0, len);

                this.Invoke((EventHandler)(delegate
                {
                    str = barcode.Remove(0, 10).Remove(1);
                    HandleBarcode(barcode);

                    if(str == "Z")
                        lb_OutputBarcode.Text = barcode;
                    else
                        lb_InputBarcode.Text = barcode;

                    if (str == "F")
                    {
                        tb_LittleRollNo.Text = barcode.Remove(0, 19).Remove(2);
                    }

                    tb_BatchNo.Text = barcode.Remove(7);
                    tb_WorkNo.Text = barcode.Remove(12);
                    tb_BigRollNo.Text = barcode.Remove(0, 16).Remove(3);
                    UpdateUserInput();
                }));
        ...
        private void UpdateUserInput()
        {
			...
			UserInput.OutputBarcode = lb_OutputBarcode.Text;
        }
