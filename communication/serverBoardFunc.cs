                    ...
                    switch (communicationType)
                    {
                        ...
                        case COMMUNICATION_TYPE_START_HANDSHAKE_WITH_ID_TO_PC:////the really Minyi handshake data packet, will show board ID. Flag3!!!!!
                            ...
                            else
                            {
                                ...
                                {
                                    //Zihua has only 18 machines, ID larger than that means we are doing some test, target the machine ID to 2
                                    if (myBoardID > gVariable.machineNameArray.Length)
                                        myBoardID = 2;
                                }
                                else
                                {
                                    if (myBoardID == 145)  //board 3 corrupted, we use this board (145) to substitute board 3
                                        myBoardID = 3;
                                    else if (myBoardID == 142)  //board 3 corrupted, we use this board (145) to substitute board 3
                                        myBoardID = 4;
                                    else if (myBoardID == 0 || myBoardID > gVariable.maxMachineNum)
                                        myBoardID = gVariable.maxMachineNum;
                                }
                                myBoardIndex = myBoardID - 1;
                            ...
                            gVariable.machineStatus[myBoardIndex].machineCode = gVariable.machineCodeArray[myBoardIndex]; //databaseNameThis;
                            gVariable.machineStatus[myBoardIndex].machineName = gVariable.machineNameArray[myBoardIndex];
