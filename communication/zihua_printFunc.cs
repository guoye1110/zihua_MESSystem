    ...
    public partial class communicate
    {
        public partial class ClientThread
        {
            public class zihua_printerClient
            {
				...
				private const int COMMUNICATION_TYPE_PRINT_PROCESS_INKUSED_UPLOAD = 0xCA;
				...
				private const int COMMUNICATION_TYPE_SLIT_PROCESS_SUPPLEMENTRYUSED_UPLOAD = 0xCB;
				...
				public delegate void notifyHandler(string barcode,int machineID);
				public static event notifyHandler notify_client;

                public int m_eventExist;
                ...
                public zihua_printerClient(ClientThread cThread)
                {
                    ...
                    m_eventExist = 0;
					m_Operator = null;
                    Console.WriteLine("a new zihua_printerClient generated, m_ClientThread.clientSocketInServer is :" + m_ClientThread.clientSocketInServer);
                }

                //We need to remove these delegate functions, otherwise connection broken, client thread terminated, but this event still exists, which will cause exception
                public void printerClientRemoveEvent()
                {
				...
				public void notify_printerClient(string barcode, int machineID)
                {
                	notify_client(barcode, machineID);
				}
                ...
                private int setMaterialWareInOut(int communicationType, byte[] onePacket, int packetLen)
                {
                    ...
                    else
                    {
						...
						{
							materiallistDB materiallist_db = new materiallistDB(Convert.ToUInt16(materialdelivery.targetMachine));
							materiallistDB.material_t? st_material;
                        	dispatchlistDB dispatchlist_db = new dispatchlistDB(Convert.ToUInt16(materialdelivery.targetMachine));;
							string dName = gVariable.DBHeadString + materialdelivery.targetMachine.PadLeft(3, '0');
                        	dispatchlistDB.dispatchlist_t[] st_dispatchArray;
							int feedbin_index = Convert.ToUInt16(materialdelivery.targetFeedBinIndex);
							
                        	st_dispatchArray = dispatchlist_db.readallrecord_Ordered();
                        	if (st_dispatchArray != null) {
								st_material = materiallist_db.readrecord_byDispatchCode(st_dispatchArray[0].dispatchCode);
								if (st_material != null) {
									materiallistDB.material_t st_materialUpdate = new materiallistDB.material_t();
									
									st_materialUpdate.materialStatus = st_material.Value.materialStatus.Substring(0,feedbin_index-1)+"0"+st_material.Value.materialStatus.Substring(feedbin_index, 7-feedbin_index);
									materiallist_db.updaterecord_ByDispatchcode(st_materialUpdate, st_dispatchArray[0].dispatchCode);
								}
                        	}
						}
                    }
				...
				private void materialBarcodeUpload(string barcode, int machineID)
                {
                    byte[] aa;

                    if (m_ClientThread.clientSocketInServer == null)
                    {
                        Console.WriteLine("m_ClientThread.clientSocketInServer is null !!!!");
                        return;
                    }

					//2018.6.15增加machineID判断，非本产线扫描的不予处理
					if (m_machineIDForPrint >= PRINT_PROCESS_PC1 && 
						m_machineIDForPrint <= PRINT_PROCESS_PC1+(gVariable.printingProcess[gVariable.printingProcess.Length-1]-gVariable.printingProcess[0]) &&
						machineID == m_machineIDForPrint-PRINT_PROCESS_PC1+gVariable.printingProcess[0])
					{
                        ...
                        gVariable.currentLargeRollIndex[machineID - 1] = Convert.ToInt32(st_print.largeIndex) + 1;
                        aa = System.Text.Encoding.Default.GetBytes(barcode.Substring(19, 1));
                        gVariable.currentRollStatus[machineID - 1] = aa[0];
                    }
					else if (m_machineIDForPrint >= SLIT_PROCESS_PC1 && 
							 m_machineIDForPrint <= SLIT_PROCESS_PC1+(gVariable.slittingProcess[gVariable.slittingProcess.Length-1]-gVariable.slittingProcess[0]) &&
							 machineID == m_machineIDForPrint-SLIT_PROCESS_PC1+gVariable.slittingProcess[0])
					{
						...
						st_slit.machineID = machineID.ToString();
                    ...
                    if (communicationType == COMMUNICATION_TYPE_CAST_PROCESS_START || communicationType == COMMUNICATION_TYPE_PRINT_PROCESS_START || communicationType == COMMUNICATION_TYPE_SLIT_PROCESS_START)
                    {
						...
						//工单动态生产（getDispatchFromTaskSheet），有且只有一个published/started工单，直到上一个工单完成（status = MACHINE_STATUS_DISPATCH_COMPLETED）
                        {
                        	...
                        	if (st_dispatchArray != null)
                        	{
								...
								//2018.06.07 打印/分切工序增加油墨/辅料 <materialRequired1>;...;<materialRequired7>
								if (communicationType == COMMUNICATION_TYPE_PRINT_PROCESS_START || communicationType == COMMUNICATION_TYPE_SLIT_PROCESS_START) {
									materiallistDB material_db = new materiallistDB(machineID);
                                    materiallistDB.material_t? material;
									material = material_db.readrecord_byDispatchCode(dispatchCode);
									str += ";" + material.Value.materialRequired1 + ";" + material.Value.materialRequired2;
									str += ";" + material.Value.materialRequired3 + ";" + material.Value.materialRequired4;
                                    str += ";" + material.Value.materialRequired5 + ";" + material.Value.materialRequired6 + ";" + material.Value.materialRequired7 + ";";
									//2018.06.09 分切工序增加<multiProduct>;<productCode>;<productCode2>;<productCode3>;<productCode4>;<batchNum>;<batchNum2>;<batchNum3>;<batchNum4>
									if (communicationType == COMMUNICATION_TYPE_SLIT_PROCESS_START) {
										str += st_dispatchArray[0].multiProduct + ";";
										str += st_dispatchArray[0].productCode + ";" + st_dispatchArray[0].productCode2 + ";" + st_dispatchArray[0].productCode3 + ";" + st_dispatchArray[0].productCode4 + ";";
										str += st_dispatchArray[0].batchNum + ";" + st_dispatchArray[0].batchNum1 + ";" + st_dispatchArray[0].batchNum2 + ";" + st_dispatchArray[0].batchNum3;
									}
								}
                ...
                private int HandleBarcode(int communicationType, byte[] onePacket, int packetLen)
                {
                    ...
                    {
                        productcastinglistDB.productcast_t st_cast = new productcastinglistDB.productcast_t();
                        productcastinglistDB db = new productcastinglistDB();

                        //barcode;weight;<ID1>;<ID2>;<ID3>;<ID4>;<班次>;<班别>;<productCode>;<customer>
                        if (strInputArray.Length != 10) return -1;
                        ...
                        if(strInputArray[1]!="")	st_cast.weight = Convert.ToSingle(strInputArray[1]);
						st_cast.errorStatus = strInputArray[0].Substring(19, 1);
						//2018.06.07 新增字段ID1，ID2，ID3， ID4，班次，班别，productCode, customer
						st_cast.operatorName = strInputArray[2];
						st_cast.operatorName2 = strInputArray[3];
						st_cast.operatorName3 = strInputArray[4];
						st_cast.operatorName4 = strInputArray[5];
						st_cast.workshift = strInputArray[6];
						st_cast.workTeam = strInputArray[7];
						st_cast.productCode = strInputArray[8];
						st_cast.customer = strInputArray[9];
                    ...
                    if (communicationType == COMMUNICATION_TYPE_PRINT_PROCESS_PRODUCT_BARCODE_UPLOAD)
                    {
                        ...
                        if(strInputArray[2]!="")	st_print.weight = Convert.ToSingle(strInputArray[2]);
                    ...
                    if (communicationType == COMMUNICATION_TYPE_SLIT_PROCESS_PRODUCT_BARCODE_UPLOAD)
                    {
                        ...
                        if(strInputArray[2]!="")	st_slit.weight = Convert.ToSingle(strInputArray[2]);
                        st_slit.plateNo = strInputArray[4];
						//2018.06.07 新增字段ID1, 班次，班别，productCode,  customer,  theoryWeight，realLength, threoryLength
						st_slit.operatorName = strInputArray[5];
						//st_slit.operatorName2 = strInputArray[6];
						st_slit.workshift = strInputArray[6];
						st_slit.workTeam = strInputArray[7];
						st_slit.productCode = strInputArray[8];
						st_slit.customer = strInputArray[9];
						if (strInputArray[10]!="")	st_slit.theoryWeigth = Convert.ToSingle(strInputArray[10]);
						if (strInputArray[11]!="")	st_slit.realLength = Convert.ToSingle(strInputArray[11]);
						if (strInputArray[12]!="")	st_slit.theoryLength = Convert.ToSingle(strInputArray[12]);
                        st_slit.materialScanTime = strInputArray[13];
                    ...
                    if (communicationType == COMMUNICATION_TYPE_INSPECTION_PROCESS_PRODUCT_BARCODE_UPLOAD)
                    {
						...
						//2018.06.07 新增字段productCode,  customer
						st_inspect.productCode = strInputArray[3];
						st_inspect.customer = strInputArray[4];
                    ...
                    if (communicationType == COMMUNICATION_TYPE_REUSE_PROCESS_BARCODE_UPLOAD)
                    {
                        ...
                        st_reuse.barcodeForReuse = strInputArray[0] + ":" + strInputArray[1] + ":" + strInputArray[2] + ":" + strInputArray[3];
                        st_reuse.rebuildDate = System.DateTime.Now.ToString();
                        if(strInputArray[3]!="")	st_reuse.rebuildNum = (float)Convert.ToDouble(strInputArray[3]);
                        if (strInputArray[4] != "") st_reuse.originalNum = (float)Convert.ToDouble(strInputArray[4]);
                ...
                private int HandleProcessEnd(int communicationType, byte[] onePacket, int packetLen)
                {
					...
					if (communicationType == COMMUNICATION_TYPE_PRINT_PROCESS_INKUSED_UPLOAD) {
						inkusedDB db = new inkusedDB();
						inkusedDB.inkused_t? st_inkused;
						string temp = (m_machineIDForPrint - PRINT_PROCESS_PC1 + gVariable.printingProcess[0]).ToString() + ";" + strInput;
						st_inkused = db.Deserialize(temp);
						if (st_inkused!=null)		return db.writerecord(st_inkused.Value);
						return 1;
					}
					if (communicationType == COMMUNICATION_TYPE_SLIT_PROCESS_SUPPLEMENTRYUSED_UPLOAD) {
						supplementryusedDB db = new supplementryusedDB();
						supplementryusedDB.supplementryused_t? st_supplementryused;
						string temp = (m_machineIDForPrint - SLIT_PROCESS_PC1 + gVariable.slittingProcess[0]).ToString() + ";" + strInput;
						st_supplementryused = db.Deserialize(temp);
						if (st_supplementryused!=null)	return db.writerecord(st_supplementryused.Value);
						return 1;
					}
				...
				private int HandlePackingProcess(int communicationType, byte[] onePacket, int packetLen)
                {
                	...
                	if (communicationType == COMMUNICATION_TYPE_SLIT_PROCESS_PACKAGE_BARCODE_UPLOAD){
                        ...
                        try
                        {
							...
							if(strInputArray[5]!="")	st_packing.rollNumber = Convert.ToUInt16(strInputArray[5]);
                            if(strInputArray[6]!="")	st_packing.totalWeight = Convert.ToSingle(strInputArray[6]);
                            if(strInputArray[7]!="")	st_packing.totalLength = Convert.ToSingle(strInputArray[7]);
                            ...
                            //2018.06.07 新增字段customer
                            st_packing.customer = strInputArray[10];
