...
//                checkHeartBeatThread = null;
                ...
                try
                {
                    ...
                    myBoardIndex = toolClass.getMachineIndexByIP(IPArray[3]);
                    myBoardID = myBoardIndex + 1;
...
//                                    Console.WriteLine(" ");
                                ...
                                if (checkBytes[0] == 'w' && checkBytes[1] == 'I' && checkBytes[2] == 'F' && checkBytes[3] == 'i' && packetLen <= MAX_PACKET_LEN && toolClass.checkCrc32Code(checkBytes, packetLen) == true)
                                {
                                    try
                                    {
                                        ...
                                        {
                                            int id = onePacket[PROTOCOL_DATA_POS] + onePacket[PROTOCOL_DATA_POS + 1] * 0x100; // +onePacket[PROTOCOL_DATA_POS + 2] * 0x10000 + onePacket[PROTOCOL_DATA_POS + 3] * 0x1000000;

                                            if(id >= SHOW_BOARD_ID_START && id < SHOW_BOARD_ID_START + 100)
                                            {
                                                m_showBoardClient.processShowBoardFunc(communicationType, onePacket, packetLen);
                                            }
                                            else if (id > PRINT_MACHINE_ID_START && id < PRINT_MACHINE_ID_START + 500)
                                            {
                                                m_printClient.processLabelPrintingFunc(communicationType, onePacket, packetLen);
                                            }
                                            else
                                            {
                                                onePacket[PROTOCOL_COMMUNICATION_STATUS_POS] = 0;
                                                sendDataToClient(onePacket, 12, communicationType);
                                            }
                                        }
                                        else
                                        {
                                            try
                                            {
                                                ...
                                                else if (communicationType == COMMUNICATION_TYPE_PUSH_DATA || communicationType == COMMUNICATION_TYPE_UPDATE_FORMAT)
                                                {
                                                    m_showBoardClient.processShowBoardFunc(communicationType, onePacket, packetLen);
                                                }
                                                ...
                                                else if (communicationType >= COMMUNICATION_TYPE_CLIENT_TOUCHPAD && communicationType <= COMMUNICATION_TYPE_CLIENT_TOUCHPAD + 0x1F)
                                                {   //0x80 - 0xaf
                                                    //communication with data collect board
                                                    debugInfoDisplay(packetLen);
                                                    processClientDataCollectBoard(communicationType, packetLen);
                                                }
                                                else //if (communicationType >= COMMUNICATION_TYPE_CLIENT_PC && communicationType <= COMMUNICATION_TYPE_CLIENT_PC + 0x1f)
                                                {
