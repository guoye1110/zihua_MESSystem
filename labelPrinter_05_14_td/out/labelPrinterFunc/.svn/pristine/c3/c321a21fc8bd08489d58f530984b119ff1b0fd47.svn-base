...
public class ChuRuKuDailyRP
{
    ...
    private int PLeft;
    private int PTop;
#pragma warning disable CS0169 // The field 'ToPrint.PRight' is never used
    private int PRight;
#pragma warning restore CS0169 // The field 'ToPrint.PRight' is never used
    private int PBottom;
    private int PWidth;
    private int PHeigh;
    //当前画笔颜色
...
#pragma warning restore CS0168 // The variable 'ex' is declared but never used
    ...
    private PrintDocument CreatePrintDocument(DataTable dt, string Title)
    {
        ...
        //  DataTablePrinter.DefaultPageSettings.PaperSize = new PaperSize("newPage70X40"
        //   , 827
        //    , 1169);
    ...
    //
    int CalcColumnWidth(PrintPageEventArgs Ev)
    {

        int tableWith = 0;
        string ColumnText;

        int columnCount = DataTablePrint.Columns.Count;
        ...
        //Caculate the Title Width
        if (PrintingPageNumber == 1)
        {
            for (int Cols = 0; Cols <= columnCount - 1; Cols++)
            {
        ...
        //Add all cell width to get the total TableWidth;
    ...
    private void DataTablePrinter_PrintPage(object sende, PrintPageEventArgs Ev)
    {
        ...
        if (PrintingPageNumber == 1 || PageRecordNumber > TotalNum)//最后一页只打印统计行时不打印列名
        {
            ...
            for (int Cols = 0; Cols <= DataTablePrint.Columns.Count - 1; Cols++)
            {
                ...
                Ev.Graphics.DrawString(ColumnText, TableFont, DrawBrush, x, stringY);
                if (Cols == 2 || Cols == 4)
                    Ev.Graphics.DrawLine(Pen, x, rowOfTop, x, rowOfTop + YUnit);
        ...
        Ev.Graphics.DrawLine(Pen, PLeft, rowOfTop + YUnit, x, rowOfTop + YUnit);
        stringY += YUnit;
        y += YUnit;
        ...
        {
            int top = rowOfTop;
            if (Cols == DataTablePrint.Columns.Count - 1)
                top = rowOfTop;
            else
                top = rowOfTop + YUnit;
            x += XUnit[Cols];

            Ev.Graphics.DrawLine(Pen, x, top, x, y);
        }
