...
#pragma warning restore CS0414 // The field 'BardCodeHooK._MiniLength' is assigned but its value is never used
        ...
        private void Timer_Elapsed(object sender, ElapsedEventArgs e)
        {
            ...
            if (gap > _TimeOut)
            {
                ...
                StopCheckGap();//停止检测,可能是扫描枪扫描结束，也可能是手工输入间隔
                               ...
                               //              }
                if(barCode.IsValid == 1)
                {
                    barCode.IsValid = 0;
                }

                //judgment of input string length is dangerous, we will deal with this later when we have time
                /*
                ...
                {//回车
                 //barCode.BarCode = strBarCode;
                    ...
                    if (BarCodeEvent != null && barCode.IsValid == 2)
                    {
        ...
        private int KeyboardHookProc(int nCode, Int32 wParam, IntPtr lParam)
        {
                ...
                if (wParam == 0x100)//WM_KEYDOWN=0x100 
                {
                    ...
                    else
                    {
...

                        {
                            ...
                            //finally
                            {
                                Log.d("TT", "last" + barCode.IsValid);
                                barCode.Time = DateTime.Now;
                                if (barCode.IsValid != 0)
                                {
                                    Log.d("TT", "return 1" + barCode.IsValid);
                                    if (barCode.IsValid == 2)
                                        barCode.IsValid = 0;
                                    return 1;
                                }
                            }
            ...
            if (barCode.IsValid != 0)
            {
                Log.d("TT", "return 2" + barCode.IsValid);
                if (barCode.IsValid == 2)
                    barCode.IsValid = 0;
                return 1;
            }
